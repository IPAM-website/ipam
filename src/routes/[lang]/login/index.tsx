...
export const useLogin = routeAction$(async (data,  { env }) => {
  ...
  zod$({
    username: z.string().min(2, "userError"),
    pwd: z.string().min(2, "passwordError")
  }
  ))
          ...
          <div class="border-1 z-10 shadow-2xl border-gray-100 rounded-3xl px-4 py-8 md:px-6 md:py-12 bg-white">
            ...
            <div class="w-[340px]  md:w-[400px] inline-flex flex-col justify-start items-center gap-6">
              ...
              <div class="flex flex-col justify-start items-start gap-4">
                ...
                {action.value?.failed && action.value.fieldErrors.username && (<div class="text-sm text-red-600 ms-2">{t(`runtime.${action.value.fieldErrors.username}`)}</div>)}
                <Password id="password" name='pwd' placeholder='Password'></Password>
                {action.value?.failed && action.value.fieldErrors.pwd && (<div class="text-sm text-red-600 ms-2">{t(`runtime.${action.value.fieldErrors.pwd}`)}</div>)}
            ...
            {(() => {
              let cls = "mt-2 p-3 rounded-md text-gray-100"
              if (action.value && action.value.type_message && action.value.type_message>1 && action.value.type_message<=4 && showMessage.value) {

                if(action.value.type_message!=1)
                  cls+=" bg-red-500";
                else
                  cls+=" bg-green-500";

                return <p class={cls}>{t(`runtime.result${action.value.type_message}`)}</p>
              }
            })()}
