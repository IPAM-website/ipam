import { $, component$, getLocale, Signal, Slot, useSignal, useStore, useTask$,  } from "@builder.io/qwik";
import { routeAction$, routeLoader$, server$, StaticGenerateHandler, useLocation, useNavigate, z, zod$ } from "@builder.io/qwik-city";
import sql from "~/../db"
import TextboxForm from "~/components/forms/formsComponents/TextboxForm";
import { ReteModel, AggregatoModel } from "~/dbModels";
...
export const getAllAggregatesByNetwork = server$(async function (idsito: number) {
    try {
        ...
        let aggregations : CustomRow[] = [];

        function nextNetwork(ip:string, prefix:number)
        {
            let networkWidth : number = Math.pow(2,32-prefix);
            let parsedIP : number[] = ip.split('.').map(x=>parseInt(x));
        ...
        prefixes.forEach(x=>{
            let flt = networks.filter(j=>j.prefissorete==x)
            flt.sort((a,b)=>{
                if(a.iprete>b.iprete)
                    return 1;
                return 0;
            })
            ...
            if(flt.length>1)
                for(let i=0;i<flt.length-1;i++)
                {
                    let space=1;
                    let baseIP = "";
                    while(i<flt.length-1 && nextNetwork(flt[i].iprete,x)==flt[i+1].iprete)
                    {
                        if(baseIP=="")
                            baseIP=flt[i].iprete;
                        space++;
...
export default component$(() => {
    ...
    useTask$(async () => {
        ...
        aggregates.value = await getAllAggregatesByNetwork(parseInt(loc.params.site)); 
                ...
                mode == "view"
                    ? (
                        <div>
                            ...
                            <Table>
                                <Dati DBTabella="aggregati" title={$localize`Lista aggregati`} dati={aggregates.value} nomeTabella={"aggregati"} deleteWhen={ddw} modifyWhen={mmw}  onReloadRef={getREF}> {/* funcReloadData={reloadData} */}
...
export const CRUDForm = component$(({ data, reloadFN }: { data?: AggregatoModel, reloadFN?: Signal<(() => void) | null> }) => {
    const lang = getLocale("en")
    const loc = useLocation();
    const nav = useNavigate();
    ...
    return (
        <>
            ...
            <div class="m-2 flex justify-center max-sm:*:my-2 *:w-1/2  gap-4 relative">
                <FormBox title="Informazioni">
                    <TextboxForm id="" placeholder={$localize`Nome`} title={$localize`Nome`} OnInput$={(e) => {
                        formData.nomeaggregato = (e.target as HTMLOptionElement).value;
                    }} />
                    <TextboxForm id="" placeholder={$localize`Descrizione`} title={$localize`Descrizione`} OnInput$={(e) => {
                        formData.descrizioneaggregato = (e.target as HTMLOptionElement).value;
                    }} />
                </FormBox>
            ...
            <div class="flex justify-center">
                ...
                }} class="bg-green-500 transition-all hover:bg-green-600 disabled:bg-green-300 rounded-md text-white p-2 mx-1 ms-4">{$localize`Conferma`}</button>
