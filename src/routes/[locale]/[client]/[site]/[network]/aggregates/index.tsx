...
import { ReteModel, VLANModel, IndirizziModel, AggregatoModel } from "~/dbModels";
...
export const getAllAggregatesByNetwork = server$(async function () {
    try {
        if (!this.query.has('network'))
            return [];
        const query = await sql`SELECT Aggregati.*, Aggregati_Rete.idrete as idretec FROM Aggregati INNER JOIN Aggregati_Rete ON Aggregati.idaggregato=Aggregati_Rete.idaggregato WHERE Aggregati.idrete=${this.query.get('network')}`
                ...
                mode == "view"
                    ? (
                        <div>
                            ...
                            <Table>
                                <Dati DBTabella="aggregati" title={$localize`Lista aggregati`} dati={aggregates.value} nomeTabella={"aggregati"} OnModify={handleModify} OnDelete={handleDelete} onReloadRef={getREF}> {/* funcReloadData={reloadData} */}
