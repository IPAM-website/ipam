...
export const getAllPrefixes = server$(async function () {
    try {
        ...
        const query = await sql`SELECT DISTINCT prefissorete FROM rete INNER JOIN siti_rete ON rete.idrete=siti_rete.idrete WHERE siti_rete.idsito=${this.params.site}`
...
export default component$(() => {
    ...
    return (
        <>
            <Title haveReturn={true} url={loc.url.pathname.split("prefixes")[0]} > {sitename.value.toString()} - {mode.charAt(0).toUpperCase() + mode.substring(1)} Aggregate</Title>
            ...
            <div>
                ...
                <Table>
                    <Dati DBTabella="prefissi" title={$localize`Lista prefissi`} dati={prefixes.value} nomeTabella={"prefissi"} onReloadRef={getREF} funcReloadData={reloadData}> {/* funcReloadData={reloadData} */}
