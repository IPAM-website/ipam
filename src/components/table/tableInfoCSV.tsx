  ...
  return (
    <>
      <div class="p-4">
            ...
            <thead>
              <tr>
                {CSVInfoDBTableMaps[tableName].keys.map((col) => (
                  <th
                    key={col}
                    class="border-b border-gray-200 dark:border-gray-700 dark:bg-cyan-950 bg-cyan-50 px-3 py-2 font-semibold text-cyan-800 dark:text-cyan-400"
                  >
                    {col}
                  </th>
                ))}
            ...
            <tbody class="text-center">
              <tr class="even:bg-gray-50">
                {CSVInfoDBTableMaps[tableName].example?.map((col) => (
                  <td key={col} class="border-b border-gray-100 dark:border-gray-700 dark:bg-gray-700 px-3 py-2">
                    {col}
                  </td>
                ))}
              </tr>
        ...
        <div class="mt-5 flex items-center gap-2 rounded border-l-4 border-cyan-400 dark:border-cyan-500 bg-cyan-50 dark:bg-cyan-200 px-4 py-2">
          <svg
            class="h-5 w-5 text-cyan-600 dark:text-cyan-700"
            fill="none"
            stroke="currentColor"
            stroke-width="2"
            viewBox="0 0 24 24"
