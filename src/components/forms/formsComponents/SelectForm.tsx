...
export default component$<SelectFormProps>(({ id, name, value, title, OnClick$, listName, disabled = false, noElementsHandler, noElementsText, noPointer=false }) => {
    ...
    return (<div class="flex flex-row items-center py-3 px-3  w-full bg-white" >
        {title && <label class="font-semibold w-24" for={id}>{title}</label>}
        <div class="relative bg-white w-full px-1" style={{ backgroundColor: disabled ? '#f5f5f5' : '', color: disabled ? '#ddd' : '' }}>
