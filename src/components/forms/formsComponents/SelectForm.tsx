...
export default component$<SelectFormProps>(({ id, name, value, OnClick$, listName, disabled = false }) => {
    ...
    return (<div class="flex flex-row items-center py-2 px-2  w-full bg-white" >
        ...
        <div class="relative bg-white w-full" style={{ backgroundColor: disabled ? '#f5f5f5' : '', color: disabled ? '#ddd' : '' }}>
            <div id={id} style={{ borderColor: clicked.value ? "#aaa" : "", userSelect: "none", cursor: disabled ? "default" : "" }} class="border relative w-full border-gray-200 p-1.5 px-3 cursor-pointer rounded-sm *:font-['Inter'] text-md flex items-center justify-start" onClick$={() => { if (disabled) return; clicked.value = !clicked.value; }}>
