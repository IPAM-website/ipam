...
export default component$<AddressBoxProps>(
  ({
    ...
    const assembleIP = $(async () => {
      ...
      const parsedIP = ip.split(".").map((segment) => parseInt(segment));

      for (const ip of parsedIP)
        if (ip < 0 || ip > 255) errors.push("Invalid IP");

      let parsedPrefix = parseInt(working_prefix);
      const networkIP = new Array(4);
      ...
      if (currentIPNetwork && currentIPNetwork != -1) {
        ...
        const parentNetworkIP = parentNetwork.iprete
          .split(".")
          .map((x) => parseInt(x));
        const parentLastIp = new Array(4);
